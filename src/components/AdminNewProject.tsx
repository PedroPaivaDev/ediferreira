...
const AdminNewProject = () => {
  ...
  const [photosObjectFiles, setPhotosObjectFiles] = React.useState<FileObjectStorage[] | null>([]);
  const [mainProjectPhoto, setMainProjectPhoto] = React.useState<FileObjectStorage>();

  function handleFilesSelect(
    event: React.ChangeEvent<HTMLInputElement>,
    setPhotosObjectFiles: React.Dispatch<React.SetStateAction<FileObjectStorage[] | null>>
  ) {
    ...
    setPhotosObjectFiles(arrayPhotoObjects);
  }
  ...
  return (
    <div className='w-full flex flex-col justify-start items-start gap-3'>
      ...
      <input
        ...
        id="newPhotosInput"
        accept="image/*"
        onChange={(e) => handleFilesSelect(e, setPhotosObjectFiles)}
        ...
        {photosObjectFiles && photosObjectFiles.map(photo =>
          ...
          >
            ...
            {mainProjectPhoto?.url.includes(photo.url) ?
              ...
              <p className='opacity-0 group-hover:opacity-100' onClick={() => setMainProjectPhoto(photo)}>
                usar esta
              </p>
            }
          </div>
